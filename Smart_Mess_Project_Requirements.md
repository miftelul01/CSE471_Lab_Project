...
| Miftelul Mehebub | **Shared House Map & Neighbourhood Knowledge Base:** A house-scoped map where residents pin every place the flat needs — kacha bazar, grocery, butcher, pharmacy, barber, tailor, hardware, gas cylinder supplier, water supplier, laundry, CNG stand, ATM, restaurants — organised by fixed categories and annotated with the house's own notes. Residents search by need or browse the map, see how recently each entry was confirmed accurate, and get walking or driving directions. Time-bound discounts attach to entries as an optional layer whose status is derived at read time. A resident joining the house inherits the whole map at once. |
...
| Mahia Tanzin | **Google Maps API Integration (Listing Discovery):** Enhances the property discovery phase by displaying real-world locations of rental listings on an embedded Google Map, so prospective tenants can calculate commute distances and evaluate the surrounding area. Scoped to listing discovery and commute evaluation only — the map of places a household already uses is the Module 2 Shared House Map. |
