 ...
 */
export function Sidebar({ role, platformName }: { role: UserRole | null; platformName: string }) {
  const pathname = usePathname();
  const groups: NavGroup[] = navFor(role);
    ...
    <aside className="hidden w-64 shrink-0 flex-col bg-bark-800 lg:flex">
      <div className="flex items-center gap-3 px-6 py-6">
        ...
        <span className="leading-tight">
          ...
          <span className="block text-xs text-white/50">Property operations</span>
        </span>
      </div>
